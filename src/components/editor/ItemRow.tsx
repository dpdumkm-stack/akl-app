        ...
        <div className="group p-6 bg-slate-800/40 rounded-[32px] relative border border-white/5 hover:border-blue-500/50 hover:shadow-2xl hover:shadow-blue-900/10 transition-all duration-300 font-['Outfit',sans-serif]">
            ...
            <div className="absolute -top-3 right-4 sm:right-6 flex gap-1.5 sm:gap-2 z-10 opacity-100 sm:opacity-0 group-hover:opacity-100 transition-opacity">
                <button 
                    ...
                    className="bg-blue-600 text-white px-3 sm:px-3 py-2 sm:py-1.5 rounded-full shadow-lg flex items-center gap-1.5 hover:bg-blue-500 transition-all scale-100 sm:scale-90 hover:scale-100 border border-white/10 active:scale-95"
                >
                    <Search className="w-3.5 h-3.5 sm:w-3 sm:h-3" />
                    <span className="text-[9px] font-black uppercase tracking-tighter">DB</span>
                </button>
                ...
                {item.masterId ? (
                    <>
                        <button 
                            ...
                            className="bg-orange-600 text-white px-3 py-2 sm:px-3 sm:py-1.5 rounded-full shadow-lg flex items-center gap-1.5 hover:bg-orange-500 transition-all scale-100 sm:scale-90 hover:scale-100 border border-white/10 active:scale-95"
                        >
                            <BookmarkPlus className="w-3.5 h-3.5 sm:w-3 sm:h-3" />
                    ...
                    <button 
                        type="button" 
                        onClick={() => onSaveMaster(item)} 
                        className="bg-emerald-600 text-white px-3 py-2 sm:px-3 sm:py-1.5 rounded-full shadow-lg flex items-center gap-1.5 hover:bg-emerald-500 transition-all scale-100 sm:scale-90 hover:scale-100 border border-white/10 active:scale-95"
                    >
                        <BookmarkPlus className="w-3.5 h-3.5 sm:w-3 sm:h-3" />
                ...
                <button 
                    ...
                    className="bg-red-600 text-white p-2 sm:p-2 rounded-full shadow-lg hover:bg-red-500 transition-all scale-100 sm:scale-90 hover:scale-100 border border-white/10 active:scale-95"
                >
            ...
            <div className="flex flex-col sm:flex-row gap-4">
                <div className="flex flex-row sm:flex-col gap-2 pr-0 sm:pr-3 border-b sm:border-b-0 sm:border-r border-white/5 pb-3 sm:pb-0 justify-center sm:justify-start">
                    <button type="button" onClick={() => onMove(item.id, 'up')} className="p-2 sm:p-1.5 text-slate-600 hover:text-blue-400 hover:bg-white/5 rounded-lg transition-all active:scale-95"><ChevronUp className="w-5 h-5 sm:w-4 sm:h-4" /></button>
                    <button type="button" onClick={() => onMove(item.id, 'down')} className="p-2 sm:p-1.5 text-slate-600 hover:text-blue-400 hover:bg-white/5 rounded-lg transition-all active:scale-95"><ChevronDown className="w-5 h-5 sm:w-4 sm:h-4" /></button>
                </div>
                        ...
                        <div className="space-y-1">
                            ...
                            <div className="relative">
                                <select 
                                    ...
                                    className="w-full bg-slate-900/80 border-2 border-white/10 rounded-xl text-base sm:text-xs p-3.5 sm:p-2.5 font-black text-white outline-none focus:border-blue-500 focus:bg-slate-900 transition-all shadow-lg appearance-none pr-8"
                                >
                        ...
                        {data.isJasaBahanMode ? (
                            <>
                                <div className="space-y-1">
                                    ...
                                    <div className="relative">
                                        <span className="absolute left-3 sm:left-2 top-1/2 -translate-y-1/2 text-[10px] sm:text-[9px] font-black text-emerald-600/50">Rp</span>
                                        <input type="text" value={item.hargaBahan === 0 ? '' : formatInputNumber(item.hargaBahan)} onChange={(e) => onUpdate(item.id, 'hargaBahan', parseInputNumber(e.target.value))} className="w-full bg-slate-900/80 border-2 border-white/10 rounded-xl text-base sm:text-[11px] pl-8 sm:pl-6 pr-3 sm:pr-2 py-3.5 sm:py-2.5 font-black text-right text-emerald-400 outline-none focus:border-emerald-500 focus:bg-slate-900 transition-all shadow-lg" placeholder="0" />
                                    </div>
                                </div>
                                <div className="space-y-1">
                                    <label className="text-[8px] font-black text-blue-500 uppercase tracking-widest block px-1 italic">Harga Jasa</label>
                                    <div className="relative">
                                        <span className="absolute left-3 sm:left-2 top-1/2 -translate-y-1/2 text-[10px] sm:text-[9px] font-black text-blue-600/50">Rp</span>
                                        <input type="text" value={item.hargaJasa === 0 ? '' : formatInputNumber(item.hargaJasa)} onChange={(e) => onUpdate(item.id, 'hargaJasa', parseInputNumber(e.target.value))} className="w-full bg-slate-900/80 border-2 border-white/10 rounded-xl text-base sm:text-[11px] pl-8 sm:pl-6 pr-3 sm:pr-2 py-3.5 sm:py-2.5 font-black text-right text-blue-400 outline-none focus:border-blue-500 focus:bg-slate-900 transition-all shadow-lg" placeholder="0" />
                                    </div>
                                </div>
                            </>
                        ) : (
                            <div className="space-y-1">
                                ...
                                <div className="relative">
                                    <span className="absolute left-3 sm:left-2.5 top-1/2 -translate-y-1/2 text-[11px] sm:text-[10px] font-black text-blue-600/50">Rp</span>
                                    <input type="text" value={item.harga === 0 ? '' : formatInputNumber(item.harga)} onChange={(e) => onUpdate(item.id, 'harga', parseInputNumber(e.target.value))} className="w-full bg-slate-900/80 border-2 border-white/10 rounded-xl text-base sm:text-xs pl-8 sm:pl-8 pr-3 py-3.5 sm:py-2.5 font-black text-right text-blue-400 outline-none focus:border-blue-500 focus:bg-slate-900 transition-all shadow-lg" placeholder="0" />
                                </div>
                            </div>
                        )}
                    </div>
