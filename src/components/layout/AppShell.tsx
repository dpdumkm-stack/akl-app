...
export default function AppShell({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="min-h-screen bg-slate-950 flex font-['Outfit',sans-serif]">
      ...
      <div className="flex-1 flex flex-col min-w-0">
        {/* HEADER - TOP BAR */}
        <header className="sticky top-0 z-30 bg-slate-950/80 backdrop-blur-xl border-b border-white/5 px-3 sm:px-4 lg:px-8 py-4">
          <div className="max-w-[1800px] mx-auto flex items-center justify-between">
             <div className="flex items-center gap-4">
                ...
                <div className="lg:hidden flex items-center gap-2">
                   ...
                   <h1 className="text-white font-black text-xs uppercase tracking-tighter italic">AKL</h1>
                </div>

                <div className="hidden md:flex items-center gap-3 bg-slate-900 px-4 py-2 rounded-xl border border-white/5">
                  <Search className="w-4 h-4 text-slate-400" />
                  <input type="text" placeholder="Cari Dokumen..." className="bg-transparent border-none outline-none text-xs font-bold text-slate-200 placeholder:text-slate-500 w-48" />
                </div>
             </div>

             <div className="flex items-center gap-2 lg:gap-4">
                <button className="hidden sm:block p-2.5 text-slate-400 hover:bg-white/5 hover:text-slate-200 rounded-xl transition-all relative">
                ...
                <button 
                  ...
                  className="lg:hidden p-2 text-red-400 bg-red-500/10 hover:bg-red-500/20 hover:text-red-300 rounded-xl transition-all flex items-center gap-2 border border-red-500/20"
                >
                ...
                <div className="flex items-center gap-3">
                  <div className="text-right hidden lg:block">
                    <p className="text-[10px] font-black text-white uppercase">{session?.user?.name || "Admin"}</p>
      ...
      <nav className="lg:hidden fixed bottom-0 left-0 right-0 z-50 bg-slate-950/95 border-t border-white/5 px-2 py-3 flex justify-around items-center shadow-[0_-10px_30px_rgba(0,0,0,0.5)] backdrop-blur-xl">
        {MENU_ITEMS.map((item) => {
          const isActive = pathname === item.href || pathname?.startsWith(`${item.href}/`);
          return (
            <Link 
            ...
            >
              <div className={`p-1.5 rounded-lg transition-all ${isActive ? "bg-blue-600/20" : ""}`}>
        ...
        {(session?.user as any)?.role === "OWNER" && (
           <Link 
            href="/settings/users"
            className={`flex flex-col items-center gap-1 px-1.5 sm:px-3 py-1 rounded-xl transition-all ${
              pathname === "/settings/users" ? "text-blue-400" : "text-slate-500 hover:text-slate-300"
            }`}
          >
            <div className={`p-1.5 rounded-lg transition-all ${pathname === "/settings/users" ? "bg-blue-600/20" : ""}`}>
