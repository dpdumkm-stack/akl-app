...
import { getInvoiceNumberAction, getGlobalSettings } from "@/app/actions";
...
import { getUniqueId, formatInvoiceNumber } from "@/lib/utils";
...
const emptyInvoice = (): InvoiceData => {
  ...
  return {
    ...
    clientAddress: "",
    items: [
      { id: getUniqueId(), description: "", quantity: 1, volume: null, satuan: "m2", unitPrice: 0 }
    ],
    subtotal: 0,
    discountAmount: 0,
    taxApplied: false,
...
export default function CreateInvoicePage() {
  ...
  useEffect(() => {
    ...
    getInvoiceNumberAction().then(res => {
      if (res.success && res.nextUrut) {
        setData(prev => ({
          ...prev,
          nomorUrut: res.nextUrut,
          invoiceNumber: formatInvoiceNumber(res.nextUrut)
        }));
      }
    });
  }, []);
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (res.success) {
        ...
        showToast(res.message || "Gagal menyimpan invoice", "error");
      }
        ...
        <div className="max-w-[1800px] mx-auto flex items-center justify-between">
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-[10px] text-slate-500 font-bold uppercase tracking-widest leading-none">Studio Invoicing B2B</p>
            </div>
          </div>
